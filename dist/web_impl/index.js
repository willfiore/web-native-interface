...
function transactionRequest(type, payload) {
    ...
    setTimeout(() => {
        switch (type) {
            case "http:request":
                http.handleHttpRequest(t, payload);
                break;
            case "app:info":
                app.handleAppInfo(t);
                break;
...
export function postTransactionResponse(transaction_id, payload) {
    window.__WNI_NATIVE_TO_WEB.transactionResponse(transaction_id, payload);
}
export function postNativeEvent(type, payload) {
